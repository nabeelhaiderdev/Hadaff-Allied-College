...
// $hdfall_post_tags = get_the_tags($pID);
$hdfall_post_categories = get_categories($pID);

$hdfall_posttitle=glide_page_title('hdfall_posttitle');
...
<div class="container-980">
	<div class="wrapper">
		...
		<div class="post-meta d-flex justify-content-between align-items-center">
			<!-- /.post-tags -->
			<?php if($hdfall_post_categories){ ?>
				<div class="post-cat">
					<?php foreach ($hdfall_post_categories as $category ) { ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('post-ctn'); ?>>
			...
			$hdfall_rp_selection_criteria = isset($fields['hdfall_rp_selection_criteria']) ? $fields['hdfall_rp_selection_criteria'] : null;
			if($hdfall_rp_selection_criteria == 'random'){
				...
				global $post;
				$hdfall_selected_posts = array();
				$hdfall_selected_posts = isset($fields['hdfall_rp_selected_posts']) ? $fields['hdfall_rp_selected_posts'] : null;
				if ( $hdfall_selected_posts ) { ?> <div class="related-posts ">
				<h3><?php _e( 'Related Posts', 'hdfall_td' ) ?></h3> <?php
								foreach ( $hdfall_selected_posts as $hdfall_post ) {
									$post = $hdfall_post;
									setup_postdata( $post );
									$pID         = $post->ID;
